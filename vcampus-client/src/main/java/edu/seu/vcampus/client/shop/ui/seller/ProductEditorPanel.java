...
public final class ProductEditorPanel extends JPanel {
    ...
    private EmbeddedEditorHost skuHost;
    ...
    public ProductEditorPanel(ShopUiKit uiKit) {
        ...
        content.add(fields, BorderLayout.NORTH); content.add(varieties, BorderLayout.CENTER);
        skuHost = new EmbeddedEditorHost(content); add(skuHost, BorderLayout.CENTER);
        clear("文具");
    }
    public void clear(String inheritedCategory) {
        ...
        skus.add(new EditableSku(null, "", java.math.BigDecimal.ZERO, 0, true, 0)); refreshSkus(); initialState = state();
    }
    public void load(ProductView product) {
        ...
        skus.clear(); for (ProductSkuView sku : product.skus()) skus.add(new EditableSku(sku.skuId(), sku.skuName(),
                sku.unitPrice(), sku.stockQuantity(), sku.active(), sku.rowVersion())); refreshSkus(); initialState = state();
    }
    ...
    private void editSku(int index) {
        ...
        skuHost.showEditor(new SkuEditorPanel(old == null ? null : old.result(), result -> {
            EditableSku value = old == null ? EditableSku.created(result) : old.with(result);
            if (old == null) skus.add(value); else skus.set(index, value);
            refreshSkus(); skuHost.completeAndClose();
        }, () -> skuHost.requestClose()));
    }
    ...
    private record EditableSku(String id, String name, java.math.BigDecimal price, long stock, boolean active, long version) {
        static EditableSku created(SkuEditorPanel.Result v) { return new EditableSku(null, v.name(), v.unitPrice(), v.stockQuantity(), v.active(), 0); }
        EditableSku with(SkuEditorPanel.Result v) { return new EditableSku(id, v.name(), v.unitPrice(), v.stockQuantity(), v.active(), version); }
        SkuEditorPanel.Result result() { return new SkuEditorPanel.Result(name, price, stock, active); }
    ...
    boolean hasChanges() { return !Objects.equals(initialState, state()); }
    private String state() { return name.getText() + "\0" + description.getText() + "\0" + skus; }
}
