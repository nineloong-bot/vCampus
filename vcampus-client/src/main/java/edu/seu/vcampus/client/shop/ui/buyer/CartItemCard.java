...
final class CartItemCard extends JPanel {
    ...
    CartItemCard(CartItemView item, ShopUiKit kit, Runnable open, IntConsumer updateQuantity,
        ...
        setName("cart-item-" + item.cartItemId());
        setLayout(new BoxLayout(this, BoxLayout.Y_AXIS));

        String base = "cart-item-" + item.cartItemId();
        JButton product = kit.navigationButton(base + ".product", item.productName() + " · " + item.skuName());
        ...
        add(named(new JLabel("小计：" + money(item.displayedUnitPrice().multiply(BigDecimal.valueOf(item.quantity())))),
                base + ".subtotal"));

        JSpinner quantity = named(new JSpinner(new SpinnerNumberModel(item.quantity(), 1, Integer.MAX_VALUE, 1)),
                base + ".quantity-input");
        ...
        update.addActionListener(event -> updateQuantity.accept((Integer) quantity.getValue()));
        remove.addActionListener(event -> removeItem.run());
        actions.add(update);
        actions.add(remove);
        add(actions);
    }
