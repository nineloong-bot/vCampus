...
public final class CommerceInstaller {
 ...
 public static void install(MainFrame frame,UserView user,ClientConnection connection) {
  install(frame,user,connection,()->{});
 }
 /** Installs commerce while retaining the main shell's expired-session handoff. */
 public static void install(MainFrame frame,UserView user,ClientConnection connection,Runnable sessionExpired) {
  ...
  CommercePanel page=new CommercePanel(new SocketCommerceTransport(connection,sessionExpired),admin);
