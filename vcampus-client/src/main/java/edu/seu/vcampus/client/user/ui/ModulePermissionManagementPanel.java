...
import edu.seu.vcampus.common.governance.*;
...
public final class ModulePermissionManagementPanel extends JPanel {
    ...
    public ModulePermissionManagementPanel(UserClientService users) { this(users, null); }
    ...
    private JPanel actionBar() {
        ...
        panel.add(state); panel.add(refresh); panel.add(assign); panel.add(swap); panel.add(remove);
        return panel;
    }

    private void wireActions() {
        refresh.addActionListener(event -> load(null));
        assign.addActionListener(event -> openAssignment());
    ...
    private void load(String successMessage) {
        ...
        try { response = users.listModuleAdministrators(); }
        ...
        response.whenComplete((result, failure) -> onEdt(() -> finishLoad(
                result, failure, successMessage)));
    }

    private void finishLoad(ModuleAdministrationSnapshot result,
            Throwable failure, String successMessage) {
        if (closed) return;
        setBusy(false);
        if (failure != null || result == null) {
            state.setText(UserErrorMessages.operation(failure, "权限分配加载失败，请重试"));
            return;
        }
        rows = result.administrators();
        model.setRowCount(0);
        for (ModuleAdministratorView row : rows) model.addRow(new Object[]{row.moduleName(),
                row.administratorRole().name(), row.loginId(), status(row), row.rowVersion()});
    ...
    private void remove() {
        ...
        execute(() -> users.removeModuleAdministrator(new RemoveModuleAdministratorCommand(
                selected.moduleCode(), selected.userId(), selected.rowVersion())), "模块管理资格已停用");
    }

    private void swap() {
        int[] selection = table.getSelectedRows();
        if (selection.length != 2) { state.setText("请选择两名管理员进行交换"); return; }
        ...
        execute(() -> users.swapModuleAdministrators(new SwapModuleAdministratorsCommand(
                first.moduleCode(), first.userId(), first.rowVersion(), second.moduleCode(),
                second.userId(), second.rowVersion())), "权限交换成功");
    }
    ...
    private void execute(java.util.function.Supplier<CompletableFuture<Void>> request,
        ...
        response.whenComplete((ignored, failure) -> onEdt(() -> {
            ...
            if (failure == null) load(successMessage);
                ...
                setBusy(false);
                state.setText(UserErrorMessages.operation(failure, "权限调整失败，请稍后重试"));
            }
        }));
    }
    ...
    @FunctionalInterface interface Confirmation { boolean confirm(String message); }
}
