...
客户端默认连接本机 `127.0.0.1:8888`。

## 五、固定演示数据

每次启动服务端都会重建 5 家营业店铺和 100 件在售商品：文具 10、图书 30、生活用品 45、药品 5、其他 10。商品/SKU、库存、销量和订单均为确定性数据。

本版不升级图片数据库：每类提供 4 个内置封面（共 20 个），使用稳定的 `builtin://shop/...` 标识并由客户端离线绘制符号占位。
...
本机已经有程序占用 `8888` 端口，最常见的原因是之前启动的 Shop Demo 服务端仍在运行。关闭旧服务端窗口后重新双击 `启动服务端.bat`。
