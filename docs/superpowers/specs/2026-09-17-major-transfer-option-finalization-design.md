...
保留同学院转专业能力。学生工作台返回当前批次所有启用、名额大于零且不是学生当前专业的招生选项，不按学院过滤。计算机学院学生因此能在候选列表中看到本学院的人工智能/软件工程和数学学院的数学与应用数学，但每个批次只能选择并申请其中一个专业。

单专业申请规则在多层保证：客户端使用单选下拉框；修改 `DRAFT` 只替换原申请的 `optionId`；服务端创建时拒绝已有同批次申请的学生；数据库继续保留 `(batchId, studentId)` 唯一索引。不允许用多份草稿、多个选项或重复请求同时占用两个专业名额。
...
- 学生候选列表包含同学院和跨学院专业，排除当前专业、未启用选项和零名额选项；同一批次创建第二份申请必须被服务和数据库拒绝。
- 客户端专业选择、就绪提示和按钮状态。
