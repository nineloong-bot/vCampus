...
选课开放状态由管理员维护的学期时间窗和 `termStatus` 共同决定；服务端必须以服务器时间判断开放状态，且 `termStatus=CLOSED` 时禁止普通选课、退课、改选、补选和重修等全部写操作。
...
| `adjustmentStartAt` | 退改补开始时间   | `DATETIME`    | 非空；晚于正常选课结束时间     |
...
| `createdAt`         | 创建时间         | `DATETIME`    | 非空；服务端写入               |
| `updatedAt`         | 最后更新时间     | `DATETIME`    | 非空；创建和修改时由服务端写入 |
...
| `rowVersion`     | 乐观锁版本号     | `LONG`        | 非空；默认 `0`                                              |
| `createdAt`      | 创建时间         | `DATETIME`    | 非空；服务端写入                                            |
| `updatedAt`      | 最后更新时间     | `DATETIME`    | 非空；创建、容量/状态/教师/时间变更和人数变化时由服务端写入 |
...
| `rowVersion`       | 乐观锁版本号 | `LONG`        | 非空；默认 `0`                                |
| `createdAt`        | 创建时间     | `DATETIME`    | 非空；首次创建选课记录时由服务端写入          |
| `updatedAt`        | 最后更新时间 | `DATETIME`    | 非空；选入、退课或重新激活时由服务端写入      |
...
并发验收默认使用 20 个请求竞争最后一个名额，测试人数必须可通过测试配置覆盖；任意配置人数下最多一人成功。Access 不依赖 `SELECT FOR UPDATE`，应用资源锁、事务和唯一索引共同保证一致性。
...
- 默认 20 个请求竞争 1 个名额仅 1 个成功，且并发请求数可通过测试配置覆盖；`enrolledCount` 与有效记录一致。
- 相同学生的并发重复选课只成功一次。
- 周次、星期和节次任一不重叠时不误判冲突。
- 退改补时间窗边界精确到服务端时间。
- 改选目标满员或冲突时原课程保持有效。
