...
导航使用基于 ID 的 `ShopNavigator`。首页和搜索路由保存当前查询条件，返回页面时恢复筛选与排序状态。

### 6.1 Shop 视觉与公共 UI 适配边界

Shop 页面遵循 `docs/superpowers/specs/2026-08-26-vcampus-ui-design-system.md`，采用纸张米白背景、学院墨绿结构色、砖红唯一主操作、中文衬线字体、直角细边框和克制留白。商城首页、搜索、详情、店铺、购物车、结算和支付结果保持同一视觉语言；商品浏览页面允许自适应三至四列商品网格，结算页面使用表单结构，购物车使用行项目与单一结算摘要。

Shop UI 只负责商品内容、业务状态、Shop 路由和操作事件，不拥有颜色、字体、边距、边框或应用外壳。页面通过构造器接收 Shop 内部的 `ShopUiKit` 语义适配接口；该接口负责创建导航入口、主按钮、次按钮、筛选区、商品卡片，以及加载、空结果、错误和断线状态视图。导航入口与砖红主操作是不同语义，公共适配器将其映射到应用外壳的墨绿导航状态。业务页面不得直接创建颜色、字体或边框，也不得在构造器内自行选择主题。

`DefaultShopUiKit` 是 `feat/shop-only` 的兼容实现，只组合标准 Swing 控件并读取 `UIManager` 当前默认值，不包含任何颜色、字体、边距或边框常量。测试通过 `ShopUiKit` 替身验证页面表达的语义角色。公共 `client.core.ui` 令牌与组件进入约定集成基线后，Demo 组合层提供 `SharedShopUiKitAdapter`，把相同语义映射到公共实现；Shop 页面本身保持不变。三者均位于 `client/shop/ui/style`，不得修改 Foundation 或 `MainFrame`。

页面结构和业务交互可在 `feat/shop-only` 独立验证。精确视觉令牌、公共状态组件和应用外壳只在其公共实现进入约定集成基线后接通。该接通点不得改变 `ShopClientPort`、Shop 路由或页面业务状态模型。

商品首页提供搜索入口、热门商品和分类商品区；商品详情提供店铺入口、可售 SKU、数量选择、加入购物车、返回原查询页和进入购物车。加入购物车成功后显示返回购物车的商品数量，并提供明确的购物车导航。
...
验证路由历史、查询恢复、EDT 安全更新、忙碌控件、过时响应保护、错误后的购物车保留、价格变化确认和支付重试。Shop UI 审计还验证业务页面不声明私有颜色、字体和边框，不修改公共外壳，并且语义 UI 适配边界可被测试替身替换。公共 UI 实现进入集成基线后，再依据共享设计系统的尺寸、缩放、页面状态和截图清单执行最终视觉验收。
