...
Shop UI 只负责商品内容、业务状态、Shop 路由和操作事件，不拥有颜色、字体、边距、边框或应用外壳。页面通过构造器接收 Shop 内部的 `ShopUiKit` 语义适配接口；该接口负责创建主按钮、次按钮、筛选区、商品卡片，以及加载、空结果、错误和断线状态视图。业务页面不得直接创建颜色、字体或边框，也不得在构造器内自行选择主题。
