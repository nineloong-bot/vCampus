...
| 角色 | 账号 | 初始密码 | 初始登录行为 |
| --- | --- | --- | --- |
| 学生 | `213242478` | `123456` | 进入学生课程中心 |
| 教师 | `DEMO_TEACHER` | `123456` | 进入教师课程中心 |
| 超级管理员 | `DEMO_ADMIN` | `123456` | 进入课程管理页面 |

## 当前可走查能力

- 课程中心作为一个全局模块嵌入共享 vCampus 外壳；内部标签按角色显示：学生为“教学班查询、我的选课、我的课表、退改补、重修”，教师为“教学班查询、教师课表”，管理员为“学期管理、选课阶段、课程目录、教学班管理、选退记录”。课程是否通过统一读取学籍模块成绩。
