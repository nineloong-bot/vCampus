...
图书罚款已接入同一钱包：读者在图书模块主动确认缴纳，流水类型为 `LIBRARY_FINE`，`orderKey` 在此类型下保存完整借阅 `loanId`（不是商城订单）。通过 `WalletFinePort` 在调用方事务内扣款并向 `SYSTEM/LIBRARY_FINE` 记账，不创建托管记录，不增加商家收入。唯一业务键为 `F:LIBRARY:<loanId>`，重复缴纳返回原收据。接口与权限见 [图书罚款说明](../library/borrowing-penalties.md)。
