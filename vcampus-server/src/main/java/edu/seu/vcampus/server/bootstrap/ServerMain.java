...
public final class ServerMain {
    ...
    private static void run(ServerConfig config) throws Exception {
        ApplicationRuntime application = createApplication(config);
        application.startMaintenance();
        MessageRouter router = application.router();
        SocketServer server = new SocketServer(config.port(), config.workerThreads(),
                config.maxConnections(), router);
        Runtime.getRuntime().addShutdownHook(new Thread(() -> { application.close(); shutdown(server); }, "vcampus-shutdown"));
    ...
    private static MessageRouter createApplicationRouter(ServerConfig config) throws Exception {
        return createApplication(config).router();
    }

    private static ApplicationRuntime createApplication(ServerConfig config) throws Exception {
        ConnectionProvider connections = () -> DriverManager.getConnection(databaseUrl(config));
        return ApplicationRuntime.create(connections, config.databaseResourceRoot(), java.time.Clock.systemUTC(),
                Duration.ofMinutes(config.sessionTimeoutMinutes()));
    }

    private static LegacyServerComposition.ServerRuntime createRuntime(ServerConfig config) {
        return LegacyServerComposition.createRuntime(config);
    }
