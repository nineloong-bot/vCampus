...
public final class CourseHandlers {
 ...
 public void register(MessageRouter r){
  ...
  r.register("COURSE_SELECTION_PHASE_LIST",read(EmptyRequest.class,Set.of("ADMIN"),(m,b)->(Serializable)new ArrayList<>(service.listSelectionPhases())));
  r.register("COURSE_SELECTION_PHASE_CREATE",write(CreateSelectionPhaseCommand.class,Set.of("ADMIN"),(m,b)->service.createSelectionPhase(b)));
  ...
  r.register("COURSE_CLASSROOM_SEARCH",read(ClassroomQuery.class,Set.of("ADMIN"),(m,b)->(Serializable)new ArrayList<>(service.searchClassrooms(b))));
  ...
  r.register("COURSE_STUDENT_COURSE_SEARCH",read(CourseSelectionQuery.class,Set.of("STUDENT"),(m,b)->service.searchStudentCourses(m.sessionToken(),b)));
  ...
  r.register("COURSE_DROP",write(DropCommand.class,Set.of("STUDENT"),(m,b)->{service.drop(m.sessionToken(),b);return EmptyResponse.INSTANCE;}));
  r.register("COURSE_ADJUSTMENT_DROP",write(DropCommand.class,Set.of("STUDENT"),(m,b)->{service.drop(m.sessionToken(),b);return EmptyResponse.INSTANCE;}));
 ...
 private static String userMessage(String code){return switch(code){case "COMMON_CONCURRENT_MODIFICATION"->"数据已被其他操作修改，请刷新后重试";case "COMMON_FORBIDDEN"->"没有执行该操作的权限，请重新登录或联系管理员";case "COURSE_SELECTION_PHASE_ALREADY_OPEN"->"已有选课阶段开放，请先关闭后再开放新阶段";case "COURSE_SELECTION_PHASE_INVALID_STATE"->"选课阶段当前状态不允许该操作";case "COURSE_TERM_NOT_ACTIVE"->"只能为当前运行的学期开放选课";case "COURSE_ALREADY_PASSED"->"该课程已通过，无需再次选课";case "COURSE_RETAKE_REQUIRED"->"该课程有未通过记录，请使用重修选课";case "COURSE_OFFERING_FULL"->"教学班容量已满，请选择其他教学班";case "COURSE_OFFERING_HAS_ENROLLMENTS"->"已有学生选课，不能修改所属学期、课程或上课安排";case "COURSE_SCHEDULE_CONFLICT"->"所选教学班与当前课表冲突，请调整选择";case "COURSE_CLASSROOM_NOT_FOUND"->"请选择匹配结果中的有效教室";case "COURSE_CLASSROOM_CAPACITY_EXCEEDED"->"教学班容量超过教室容量，请更换教室或调低容量";case "COURSE_CLASSROOM_CONFLICT"->"该教室在所选星期、周次和节次已有教学班";case "COURSE_CLASSROOM_SPORTS_ONLY"->"桃园操场仅供体育课程使用";case "COURSE_TEACHER_CONFLICT"->"该教师在所选星期、周次和节次已有教学班";case "COURSE_DUPLICATE_ENROLLMENT"->"本学期已选择该课程，请先查看当前选课";case "COURSE_ENROLLMENT_NOT_OPEN"->"当前未开放选课阶段";case "COURSE_DROP_NOT_OPEN"->"当前未开放可退选的选课阶段";case "COURSE_ADJUSTMENT_NOT_OPEN"->"当前未开放退改补选阶段";case "COURSE_STUDENT_INELIGIBLE"->"当前学籍状态不允许选课，请联系教务人员";case "COURSE_ENROLLMENT_NOT_ACTIVE"->"选课记录已失效，请刷新后重试";case "COURSE_CHANGE_TARGET_INVALID"->"目标教学班不可用于改选，请重新选择";case "COURSE_RETAKE_NOT_ELIGIBLE"->"未找到不通过的修读记录，暂不能重修";case "COURSE_OUTCOME_IMPORT_INVALID"->"导入内容无效或来源冲突，请核对后重试";default->"操作未完成，请刷新数据后重试";};}
}
