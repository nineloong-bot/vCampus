...
public final class CourseHandlers {
 ...
 public void register(MessageRouter r){
  ...
  r.register("COURSE_ENROLL",write(EnrollCommand.class,Set.of("STUDENT","ADMIN"),(m,b)->service.enroll(m.sessionToken(),b)));
  r.register("COURSE_ADJUSTMENT_ADD",write(LateAddCommand.class,Set.of("STUDENT","ADMIN"),(m,b)->service.addDuringAdjustment(m.sessionToken(),b)));
  r.register("COURSE_ADJUSTMENT_DROP",write(DropCommand.class,Set.of("STUDENT","ADMIN"),(m,b)->{service.dropDuringAdjustment(m.sessionToken(),b);return EmptyResponse.INSTANCE;}));
  r.register("COURSE_ADJUSTMENT_CHANGE",write(ChangeOfferingCommand.class,Set.of("STUDENT","ADMIN"),(m,b)->service.changeDuringAdjustment(m.sessionToken(),b)));
  r.register("COURSE_RETAKE_CHECK",read(EntityIdRequest.class,Set.of("STUDENT","ADMIN"),(m,b)->service.checkRetakeEligibility(m.sessionToken(),b.entityId())));
  r.register("COURSE_RETAKE_ENROLL",write(RetakeCommand.class,Set.of("STUDENT","ADMIN"),(m,b)->service.enrollRetake(m.sessionToken(),b)));
  r.register("COURSE_GET_MY_SCHEDULE",read(EmptyRequest.class,Set.of("STUDENT","TEACHER","ADMIN"),(m,b)->(Serializable)new ArrayList<>(service.getCurrentSchedule(m.sessionToken()))));
  r.register("COURSE_GET_MY_ENROLLMENTS",read(EmptyRequest.class,Set.of("STUDENT","ADMIN"),(m,b)->(Serializable)new ArrayList<>(service.getCurrentEnrollments(m.sessionToken()))));
 ...
 private CourseSessionIdentity requireRole(Message m,Set<String> roles){if(m.sessionToken()==null||m.sessionToken().isBlank())throw new CourseForbiddenException();CourseSessionIdentity id=authorization.requireSession(m.sessionToken());if(id==null||!roles.contains(id.role()))throw new CourseForbiddenException();return id;}
