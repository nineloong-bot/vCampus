...
import edu.seu.vcampus.server.course.domain.ClassroomSchedulingPolicy;
...
public final class CourseServiceImpl implements CourseService, CourseQueryPort {
    ...
    private final ClassroomSchedulingPolicy classroomPolicy;
    ...
    public CourseServiceImpl(CourseAuthorizationGateway authorization,
        ...
        this.classroomPolicy = new ClassroomSchedulingPolicy(repository);
    ...
    @Override public PageResult<CourseView> searchCatalog(CourseCatalogQuery q){return transactions.inTransaction(c->{String department=q.departmentName()==null?"":q.departmentName().strip().toLowerCase(java.util.Locale.ROOT);var all=repository.findCourses(c).stream().filter(x->q.keyword()==null||x.courseCode().contains(q.keyword())||x.courseName().contains(q.keyword())).filter(x->department.isEmpty()||x.departmentName()!=null&&x.departmentName().toLowerCase(java.util.Locale.ROOT).contains(department)).filter(x->!Boolean.TRUE.equals(q.activeOnly())||x.active()).map(CourseServiceImpl::toView).toList();int from=Math.min(all.size(),q.page()*q.pageSize());return new PageResult<>(all.subList(from,Math.min(all.size(),from+q.pageSize())),q.page(),q.pageSize(),all.size());});}
    @Override public List<ClassroomView> searchClassrooms(ClassroomQuery q){return transactions.inTransaction(c->repository.findClassrooms(c,q.keyword(),q.minimumCapacity(),q.limit()).stream().map(x->new ClassroomView(x.classroom(),x.capacity())).toList());}
    ...
    @Override public CourseView createCourse(CreateCourseCommand command){Objects.requireNonNull(command);return locks.withLocks(List.of(new ResourceKey("CURRICULUM_COURSE",String.valueOf(command.planCourseId()))),()->transactions.inTransaction(c->{if(!enforceCurriculumCatalog||!curriculumCandidates.supports(c))return toView(repository.insertCourse(c,new edu.seu.vcampus.server.course.repository.Course(null,command.courseCode(),command.courseName(),command.credit(),command.totalHours(),command.description(),command.active(),0,null,null)));var source=curriculumCandidates.requireAvailable(c,command.planCourseId());return toView(repository.insertCourse(c,new edu.seu.vcampus.server.course.repository.Course(null,source.courseCode(),source.courseName(),source.departmentId(),source.departmentName(),source.credits(),source.totalHours(),command.description(),command.active(),0,null,null)));}));}
    @Override public CourseView updateCourse(UpdateCourseCommand command){Objects.requireNonNull(command);return locks.withLocks(List.of(new ResourceKey("COURSE",command.courseId())),()->transactions.inTransaction(c->{var old=repository.requireCourse(c,command.courseId());return toView(repository.updateCourse(c,new edu.seu.vcampus.server.course.repository.Course(old.courseId(),enforceCurriculumCatalog?old.courseCode():command.courseCode(),enforceCurriculumCatalog?old.courseName():command.courseName(),old.departmentId(),old.departmentName(),enforceCurriculumCatalog?old.credit():command.credit(),enforceCurriculumCatalog?old.totalHours():command.totalHours(),command.description(),command.active(),old.rowVersion(),old.createdAt(),old.updatedAt()),command.expectedVersion()));}));}
    @Override public OfferingView createOffering(CreateOfferingCommand command){Objects.requireNonNull(command);return locks.withLocks(List.of(new ResourceKey("CLASSROOM_SCHEDULE","GLOBAL"),new ResourceKey("OFFERING_CREATE",command.courseId()+":"+command.className())),()->{authorization.requireUserRole(command.teacherUserId(),"TEACHER");return transactions.inTransaction(c->{var term=activeTerm(c);var course=repository.requireCourse(c,command.courseId());var schedules=toSchedules(null,command.schedules());classroomPolicy.validate(c,term.termId(),null,command.capacity(),schedules);var value=new Offering(null,term.termId(),command.courseId(),command.teacherUserId(),command.className(),command.capacity(),0,command.offeringStatus(),0,null,null);var saved=repository.insertOffering(c,value,schedules);repository.saveRetakeCapacity(c,saved.offeringId(),command.retakeCapacity());return toView(saved,repository.findSchedules(c,saved.offeringId()),course,repository.findRetakeQuota(c,saved.offeringId()));});});}
    @Override public OfferingView updateOffering(UpdateOfferingCommand command){Objects.requireNonNull(command);return locks.withLocks(List.of(new ResourceKey("CLASSROOM_SCHEDULE","GLOBAL"),new ResourceKey("OFFERING",command.offeringId())),()->{authorization.requireUserRole(command.teacherUserId(),"TEACHER");return transactions.inTransaction(c->{var old=repository.requireOffering(c,command.offeringId());var course=repository.requireCourse(c,command.courseId());var schedules=toSchedules(command.offeringId(),command.schedules());classroomPolicy.validate(c,old.termId(),old.offeringId(),command.capacity(),schedules);if(repository.existsEnrollmentForOffering(c,old.offeringId())&&structuralOfferingChange(c,old,command,schedules))throw new OfferingHasEnrollmentsException();if(command.capacity()<old.enrolledCount())throw new IllegalArgumentException("capacity below enrolled count");var quota=repository.findRetakeQuota(c,old.offeringId());if(command.retakeCapacity()<quota.enrolledCount())throw new IllegalArgumentException("retake capacity below enrolled count");var value=new Offering(old.offeringId(),old.termId(),command.courseId(),command.teacherUserId(),command.className(),command.capacity(),old.enrolledCount(),command.offeringStatus(),old.rowVersion(),old.createdAt(),old.updatedAt());var saved=repository.updateOffering(c,value,command.expectedVersion(),schedules);var updatedQuota=repository.saveRetakeCapacity(c,saved.offeringId(),command.retakeCapacity());return toView(saved,repository.findSchedules(c,saved.offeringId()),course,updatedQuota);});});}
    ...
    private OfferingSummary summary(Connection c,Offering o,edu.seu.vcampus.server.course.repository.Course course){var quota=repository.findRetakeQuota(c,o.offeringId());return new OfferingSummary(o.offeringId(),o.termId(),o.courseId(),course.courseCode(),course.courseName(),course.departmentName(),o.teacherUserId(),authorization.userDisplayName(o.teacherUserId()),o.className(),o.capacity(),o.enrolledCount(),quota.capacity(),quota.enrolledCount(),o.offeringStatus(),o.rowVersion(),toScheduleItems(repository.findSchedules(c,o.offeringId()),o,course));}
    ...
    private edu.seu.vcampus.server.course.repository.Term activeTerm(Connection connection) {
        List<edu.seu.vcampus.server.course.repository.Term> active = repository.findTerms(connection)
                .stream().filter(term -> "ACTIVE".equals(term.termStatus())).toList();
    ...
    private boolean structuralOfferingChange(Connection connection, Offering old,
        ...
        if (!old.courseId().equals(command.courseId())) return true;
