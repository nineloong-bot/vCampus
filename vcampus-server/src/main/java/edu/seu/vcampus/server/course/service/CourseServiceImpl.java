...
import edu.seu.vcampus.server.course.domain.OfferingSchedulingPolicy;
...
public final class CourseServiceImpl implements CourseService, CourseQueryPort {
    ...
    private final OfferingSchedulingPolicy schedulingPolicy;
    ...
    public CourseServiceImpl(CourseAuthorizationGateway authorization,
        ...
        this.schedulingPolicy = new OfferingSchedulingPolicy(repository);
    ...
    @Override public List<ClassroomView> searchClassrooms(ClassroomQuery q){Objects.requireNonNull(q);return transactions.inTransaction(c->repository.findClassrooms(c,q.keyword(),q.minimumCapacity(),q.limit()).stream().map(x->new ClassroomView(x.classroom(),x.capacity(),x.sharedSportsVenue())).toList());}
    ...
    @Override public OfferingView createOffering(CreateOfferingCommand command){Objects.requireNonNull(command);return locks.withLocks(List.of(new ResourceKey("OFFERING_SCHEDULE","GLOBAL"),new ResourceKey("OFFERING_CREATE",command.courseId()+":"+command.className())),()->{authorization.requireUserRole(command.teacherUserId(),"TEACHER");return transactions.inTransaction(c->{var term=activeTerm(c);var course=repository.requireCourse(c,command.courseId());var schedules=toSchedules(null,command.schedules());schedulingPolicy.validate(c,term.termId(),null,command.teacherUserId(),physicalEducation(course),command.capacity(),schedules);var value=new Offering(null,term.termId(),command.courseId(),command.teacherUserId(),command.className(),command.capacity(),0,command.offeringStatus(),0,null,null);var saved=repository.insertOffering(c,value,schedules);repository.saveRetakeCapacity(c,saved.offeringId(),command.retakeCapacity());return toView(saved,repository.findSchedules(c,saved.offeringId()),course,repository.findRetakeQuota(c,saved.offeringId()));});});}
    @Override public OfferingView updateOffering(UpdateOfferingCommand command){Objects.requireNonNull(command);return locks.withLocks(List.of(new ResourceKey("OFFERING_SCHEDULE","GLOBAL"),new ResourceKey("OFFERING",command.offeringId())),()->{authorization.requireUserRole(command.teacherUserId(),"TEACHER");return transactions.inTransaction(c->{var old=repository.requireOffering(c,command.offeringId());var course=repository.requireCourse(c,command.courseId());var schedules=toSchedules(command.offeringId(),command.schedules());if(repository.existsEnrollmentForOffering(c,old.offeringId())&&structuralOfferingChange(c,old,command,schedules))throw new OfferingHasEnrollmentsException();if(command.capacity()<old.enrolledCount())throw new IllegalArgumentException("capacity below enrolled count");var quota=repository.findRetakeQuota(c,old.offeringId());if(command.retakeCapacity()<quota.enrolledCount())throw new IllegalArgumentException("retake capacity below enrolled count");schedulingPolicy.validate(c,old.termId(),old.offeringId(),command.teacherUserId(),physicalEducation(course),command.capacity(),schedules);var value=new Offering(old.offeringId(),old.termId(),command.courseId(),command.teacherUserId(),command.className(),command.capacity(),old.enrolledCount(),command.offeringStatus(),old.rowVersion(),old.createdAt(),old.updatedAt());var saved=repository.updateOffering(c,value,command.expectedVersion(),schedules);var updatedQuota=repository.saveRetakeCapacity(c,saved.offeringId(),command.retakeCapacity());return toView(saved,repository.findSchedules(c,saved.offeringId()),course,updatedQuota);});});}
    ...
    private static boolean physicalEducation(edu.seu.vcampus.server.course.repository.Course course){return course.courseName().startsWith("体育");}
