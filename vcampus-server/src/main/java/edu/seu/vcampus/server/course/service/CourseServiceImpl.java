...
public final class CourseServiceImpl implements CourseService, CourseQueryPort {
    ...
    @Override public OfferingView createOffering(CreateOfferingCommand command){Objects.requireNonNull(command);return locks.withLocks(List.of(new ResourceKey("OFFERING_CREATE",command.courseId()+":"+command.className())),()->{authorization.requireUserRole(command.teacherUserId(),"TEACHER");return transactions.inTransaction(c->{var term=activeTerm(c);var course=repository.requireCourse(c,command.courseId());var value=new Offering(null,term.termId(),command.courseId(),command.teacherUserId(),command.className(),command.capacity(),0,command.offeringStatus(),0,null,null);var saved=repository.insertOffering(c,value,toSchedules(null,command.schedules()));repository.saveRetakeCapacity(c,saved.offeringId(),command.retakeCapacity());return toView(saved,repository.findSchedules(c,saved.offeringId()),course,repository.findRetakeQuota(c,saved.offeringId()));});});}
    ...
    @Override public PageResult<OfferingSummary> searchOfferings(OfferingSearchQuery query){Objects.requireNonNull(query);DayOfWeek day=query.dayOfWeek()==null||query.dayOfWeek().isBlank()?null:DayOfWeek.valueOf(query.dayOfWeek().toUpperCase());return transactions.inTransaction(c->{var page=repository.searchOfferings(c,new edu.seu.vcampus.server.course.repository.OfferingSearchCriteria(query.termId(),query.keyword(),day,Boolean.TRUE.equals(query.availableOnly()),query.page(),query.pageSize()));List<OfferingSummary> items=new ArrayList<>();for(var o:page.items()){var course=repository.requireCourse(c,o.courseId());var quota=repository.findRetakeQuota(c,o.offeringId());items.add(new OfferingSummary(o.offeringId(),o.termId(),o.courseId(),course.courseCode(),course.courseName(),course.departmentName(),o.teacherUserId(),o.className(),o.capacity(),o.enrolledCount(),quota.capacity(),quota.enrolledCount(),o.offeringStatus(),o.rowVersion(),toScheduleItems(repository.findSchedules(c,o.offeringId()),o,course)));}return new PageResult<>(items,query.page(),query.pageSize(),page.total());});}
    ...
    private OfferingSummary summary(Connection c,Offering o,edu.seu.vcampus.server.course.repository.Course course){var quota=repository.findRetakeQuota(c,o.offeringId());return new OfferingSummary(o.offeringId(),o.termId(),o.courseId(),course.courseCode(),course.courseName(),course.departmentName(),o.teacherUserId(),o.className(),o.capacity(),o.enrolledCount(),quota.capacity(),quota.enrolledCount(),o.offeringStatus(),o.rowVersion(),toScheduleItems(repository.findSchedules(c,o.offeringId()),o,course));}
