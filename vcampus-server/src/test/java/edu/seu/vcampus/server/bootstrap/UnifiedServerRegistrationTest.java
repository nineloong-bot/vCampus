...
class UnifiedServerRegistrationTest {
    ...
    void registersEveryCampusModuleOnOneRouter() throws Exception {
        ...
        assertThat(runtime.router().isRegistered("SHOP_SIMULATE_PAYMENT")).isFalse();
        assertThat(runtime.router().isRegistered("SHOP2_CATALOG_LIST")).isTrue();
        assertThat(runtime.router().isRegistered("SHOP2_ORDER_PAY")).isTrue();
        assertThat(runtime.router().isRegistered("SHOP2_GOV_ACTION")).isTrue();
        assertThat(runtime.router().isRegistered("WALLET_GET_BALANCE")).isTrue();
        assertThat(runtime.router().isRegistered("WALLET_RECHARGE")).isTrue();
        assertThat(runtime.router().isRegistered("WALLET_GET_HISTORY")).isTrue();
    }
